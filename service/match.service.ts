import { performanceData } from "../models/performance";
import mongoose from "mongoose";
import { PlayerEntity } from "../entity/playerEntity";
import { PerformanceEntity } from "../entity/performaceEntity";
import { MatchEntity } from "../entity/matchEntity";
import { ObjectId } from "mongodb";

export class matchService {

  static performance = async (data: any) => {
    console.log(data);

    try {
      let player_team = await PlayerEntity.findById(
        data.batterId,
        {
          team_id: 1,
          _id: 0,
        },
        {}
      );

      let findPlayer1 = await PerformanceEntity.find(
        {
          player_id: data.batterId,
        },
        {},
        {}
      );
      let findPlayer2 = await PerformanceEntity.find(
        {
          player_id: data.bowlerId,
        },
        {},
        {}
      );

      if (findPlayer1.length == 0 && findPlayer2.length == 0) {
        await PerformanceEntity.insertMany(
          [
            {
              match_id: new mongoose.Types.ObjectId(data.match_id),
              player_id: new mongoose.Types.ObjectId(data.batterId),
            },
            {
              match_id: new mongoose.Types.ObjectId(data.match_id),
              player_id: new mongoose.Types.ObjectId(data.bowlerId),
            },
          ],
          {}
        );
      } else if (findPlayer1.length == 0) {
        await PerformanceEntity.insertMany(
          [
            {
              match_id: new mongoose.Types.ObjectId(data.match_id),
              player_id: new mongoose.Types.ObjectId(data.batterId),
            },
          ],
          {}
        );
      } else if (findPlayer2.length == 0) {
        await PerformanceEntity.insertMany(
          [
            {
              match_id: new mongoose.Types.ObjectId(data.match_id),
              player_id: new mongoose.Types.ObjectId(data.bowlerId),
            },
          ],
          {}
        );
      }

      try {
        if (!data.noBall && !data.wide) {
          if (player_team.team_id.equals(data.battingTeam)) {
            console.log("batter");

            await PerformanceEntity.updateOne(
              { player_id: data.batterId },
              {
                $inc: {
                  Runs: data.Runs || 0,
                  bowlsPlayed: data.ball || 0,
                  fours: data.four || 0,
                  Six: data.Six || 0,
                },
              },
              {}
            );
          }

          await PerformanceEntity.updateOne(
            { player_id: data.bowlerId },
            {
              $inc: {
                balls: data.ball || 0,
                runsGiven: data.Runs || 0,
              },
            },
            {}
          );
        } else if (data.noBall > 0) {
          await PerformanceEntity.updateOne(
            { player_id: data.batterId },
            {
              $inc: {
                Runs: data.Runs || 0,
                fours: data.four || 0,
                Six: data.Six || 0,
              },
            },
            {}
          );

          await PerformanceEntity.updateOne(
            { player_id: data.bowlerId },
            {
              $inc: {
                runsGiven: data.Runs + data.noBall || 0,
              },
            },
            {}
          );
        } else if (data.wide > 0) {
          await PerformanceEntity.updateOne(
            { player_id: data.bowlerId },
            {
              $inc: {
                balls: data.ball || 0,
              },
            },
            {}
          );
        }

        if (data.wicket != 0) {
          await PerformanceEntity.updateOne(
            { player_id: data.bowlerId },
            {
              $push: { wickets: { player_id: data.batterId } },
            },
            {}
          );
        }
      } catch (error) {
        return error;
      }
      return "Success";
    } catch (error) {
      return error;
    }
  };


  static createMatch = async() =>{

    let createMatch = await MatchEntity.insertMany(
      [
        {
          teamA: {
            team_id: new ObjectId("64cc910cde9efedc577a2e97"),
            Extras: {},
          },
          teamB: {
            team_id: new ObjectId("64cc9230de9efedc577a2e99"),
            Extras: {},
          },
          date: new Date(),
          venue: "Mumbai",
          currentBatting: new ObjectId("64cc9230de9efedc577a2e99"),
          performance: [],
        },
      ],
      {}
    );

    return createMatch

  }
}
